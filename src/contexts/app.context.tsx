...
import { IContextType, ScreenshotConfig, TYPE_PROVIDER, ComputerUseConfig } from "@/types";
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Extab API State
  const [extabApiEnabled, setExtabApiEnabledState] = useState<boolean>(
  ...
  const loadData = () => {
    ...
    const savedComputerUseConfig = safeLocalStorage.getItem(
      STORAGE_KEYS.COMPUTER_USE_CONFIG
    );
    if (savedComputerUseConfig) {
      try {
        const parsed = JSON.parse(savedComputerUseConfig);
        if (typeof parsed === "object" && parsed !== null) {
          setComputerUseConfig({
            enabled: parsed.enabled !== undefined ? parsed.enabled : false,
            displayWidth: parsed.displayWidth || 1024,
            displayHeight: parsed.displayHeight || 768,
            model: parsed.model || "claude-sonnet-4-5",
            maxIterations: parsed.maxIterations || 10,
            showLiveFeedback: parsed.showLiveFeedback !== undefined ? parsed.showLiveFeedback : true,
          });
        }
      } catch {
        console.warn("Failed to parse computer use configuration");
      }
    }
    ...
    // Load Extab API enabled state
    const savedExtabApiEnabled = safeLocalStorage.getItem(
      STORAGE_KEYS.EXTAB_API_ENABLED
    );
    if (savedExtabApiEnabled !== null) {
      setExtabApiEnabledState(savedExtabApiEnabled === "true");
    }
  };
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (
        ...
        e.key === STORAGE_KEYS.COMPUTER_USE_CONFIG ||
  ...
  // Sync computer use config to localStorage
  useEffect(() => {
    safeLocalStorage.setItem(
      STORAGE_KEYS.COMPUTER_USE_CONFIG,
      JSON.stringify(computerUseConfig)
    );
  }, [computerUseConfig]);
  ...
  const setExtabApiEnabled = (enabled: boolean) => {
    setExtabApiEnabledState(enabled);
    safeLocalStorage.setItem(STORAGE_KEYS.EXTAB_API_ENABLED, String(enabled));
    loadData();
  };
