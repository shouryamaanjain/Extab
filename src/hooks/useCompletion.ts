...
import { computerUseAgentLoop } from "@/lib/functions/computer-use.function";
      ...
      try {
        ...
        try {
          ...
          if (computerUseConfig.enabled) {
            // Get API key from provider variables (note: stored as lowercase)
            const apiKey = selectedAIProvider.variables?.api_key || "";

            if (!apiKey) {
              setState((prev) => ({
                ...prev,
                error: "API key required for computer use mode. Please configure Claude (Anthropic) provider with your API key.",
              }));
              return;
            }

            // Use computer use agent loop
            for await (const event of computerUseAgentLoop(
              input,
              computerUseConfig,
              apiKey,
              systemPrompt,
              (action, params) => {
                ...
                if (computerUseConfig.showLiveFeedback) {
                  console.log("Computer action:", action, params);
                }
              }
            )) {
              if (event.type === "text") {
              ...
              } else if (event.type === "action" || event.type === "thinking") {
                // Show action/thinking feedback
                if (computerUseConfig.showLiveFeedback) {
            ...
            // Use the standard fetchAIResponse function
